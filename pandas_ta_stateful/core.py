...
from pandas_ta_stateful._typing import *
from pandas_ta_stateful import *
...
class AnalysisIndicators(object):
    ...
    def indicators(self, as_list: bool = None, exclude: ListStr = None) -> TextIO | ListStr:
        ...
        if indicator_count > 0:
            from pandas_ta_stateful.candle.cdl_pattern import ALL_PATTERNS
    ...
    def study_stateful(self, *args: Args, **kwargs: DictLike):
        """study_stateful

        Apply stateful indicators using a rolling state dict.

        Parameters:
            state (dict): Existing indicator state. Default: ```{}```
                If ```None```, seed state is generated from historical data.
            state_timestamp (Any): Timestamp of the last valid state. Only rows
                with index > state_timestamp are updated. Ignored when
                ```state``` is ```None```. Default: ```None```
            returns (bool): Return DataFrame. Default: ```False```
            returns_state (bool): Return state dict. Default: ```True```
            stateful_only (bool): Skip unsupported indicators if ```True```.
                Default: ```False```
            verbose (bool): Print warnings and info. Default: ```False```
            timed (bool): Print process time. Default: ```False```

        Notes:
            * Multiprocessing is not supported in stateful mode.
            * Only a subset of indicators are supported. Use
              ```pandas_ta.stateful.stateful_supported_kinds()```.
        """
        kwargs.setdefault("append", True)
        append = kwargs.get("append", True)

        returns = kwargs.pop("returns", False)
        returns_state = kwargs.pop("returns_state", True)
        state_in = kwargs.pop("state", None)
        seed_mode = state_in is None
        ...
        # Ignore multiprocessing and ordering options in stateful mode
        kwargs.pop("cores", None)
        kwargs.pop("chunksize", None)
        kwargs.pop("ordered", None)

        if seed_mode and state_timestamp is not None:
            if verbose:
                print("[i] stateful: state_timestamp ignored when state=None (seed_mode).")
        ...
        if mode["category"]:
            ...
            if ta is None:
                print(f"[X] Study not available.")
                return None if not returns_state else (None, state)
            [ta.remove(x) for x in excluded if x in ta]
            specs = [{"kind": ind, **kwargs} for ind in ta]
        elif mode["custom"]:
            ta = args[0].ta if len(args) > 0 and isinstance(args[0], Study) else None
            if ta is None:
                print(f"[X] Study not available.")
                return None if not returns_state else (None, state)
            specs = []
            for ind in ta:
                ...
                if "params" in spec and spec["params"]:
                    if verbose:
                        print(f"[i] stateful: ignoring 'params' for {spec.get('kind', 'unknown')}")
                spec.update(kwargs)
                specs.append(spec)
        elif mode["all"]:
            ta = self.indicators(as_list=True, exclude=excluded)
        ...
        for spec in specs:
            ...
            if not isinstance(kind, str):
                warnings.append("[i] stateful: skipping indicator with no kind.")
                continue
            kind = kind.lower()
            indicator = combined_registry.get(kind)
            if indicator is None:
                if stateful_only:
                    warnings.append(f"[i] stateful: skipping unsupported '{kind}'.")
                    continue
                vectorized_specs.append(spec)
                continue

            is_lookahead = kind in lookahead_kinds

            # Resolve inputs
            input_arrays = {}
            input_series = {}
            missing = False
            for inp in indicator.inputs:
                src = spec.get(inp, inp)
                series = src if isinstance(src, Series) else self._get_column(src)
                if series is None:
                    warnings.append(f"[X] stateful: missing input '{inp}' for {kind}.")
                    missing = True
                    break
                input_arrays[inp] = series.to_numpy()
                input_series[inp] = series
            if missing:
                continue

            state_key = spec.get("state_key")
            if not state_key:
                state_key = build_state_key(kind, spec)

            # Seed items are always collected for stateful + lookahead
            seed_items.append({
                "kind": kind,
                "indicator": indicator,
            ...
            if seed_mode and state_timestamp is None and not is_lookahead and not stateful_only:
                ...
                # but replay_only indicators must run statefully to produce outputs.
                if kind in replay_only_kinds:
                    use_stateful_update = True
                else:
            ...
            # Output names with prefix/suffix/col_names
            base_names = indicator.output_names(spec)
            output_names, msg = resolve_output_names(base_names, spec)
            if output_names is None:
                warnings.append(msg)
                continue

            if state_key not in state:
                state[state_key] = indicator.init(spec)
        ...
        if len(work) == 0 and len(vectorized_specs) == 0:
            if verbose and warnings:
                for msg in warnings:
                    print(msg)
            if timed:
                print(f"[i] Pandas TA stateful time: {final_time(stime)}")
            if returns_state:
                return (None, state) if returns else (DataFrame(), state)
            return None if returns else DataFrame()
        ...
        if len(work) > 0:
            for item in work:
                for name in item["output_names"]:
                    outputs[name] = [nan] * rows
        work_state_keys = {item["state_key"] for item in work}
        ...
        if state_timestamp is not None:
            try:
                idx = self._df.index
                if idx.dtype.name.startswith("datetime"):
                    from pandas import Timestamp, to_datetime
                    if not isinstance(state_timestamp, Timestamp):
                        state_timestamp = to_datetime(state_timestamp)
                mask = idx > state_timestamp
                if hasattr(mask, "to_numpy"):
                    nz = mask.to_numpy().nonzero()[0]
                else:
                    nz = [i for i, v in enumerate(mask) if v]
                if len(nz) == 0:
                    if verbose:
                        print("[i] stateful: no rows after state_timestamp; skipping updates.")
                    no_rows_after = True
                    start_idx = rows
                else:
                    start_idx = int(nz[0])
                ...
                if verbose:
                    print(f"[i] stateful: failed to apply state_timestamp: {exc}")

        if state_timestamp is not None and not seed_mode and state == {} and verbose:
            print("[i] stateful: state_timestamp provided but state is empty; results may be invalid.")

        # Seed state from historical data when state is None
        if seed_mode:
            seed_end = start_idx if state_timestamp is not None else rows
            for item in seed_items:
                if start_idx == 0 and item["state_key"] in work_state_keys:
                    # We'll replay from the beginning; no need to seed.
                    continue
                kind = item["kind"]
                seed_fn = SEED_REGISTRY.get(kind)
                seed_series = {
                    inp: item["input_series"][inp].iloc[:seed_end]
                ...
                if not item["is_lookahead"] and kind not in replay_only_kinds:
                    ...
                    try:
                        params = item["params"].get("params", ())
                        if not isinstance(params, tuple):
                            params = tuple()
                        result = getattr(self, kind)(*params, **call_spec)
                        if isinstance(result, Series):
                            seed_series[result.name] = result.iloc[:seed_end]
                        elif isinstance(result, DataFrame):
                            for col in result.columns:
                                seed_series[col] = result[col].iloc[:seed_end]
                    except Exception as exc:
                        if verbose:
                            warnings.append(f"[i] stateful seed: vectorized calc failed for '{kind}': {exc}")

                try:
                    if seed_fn is not None:
                        state[item["state_key"]] = seed_fn(seed_series, item["params"])
                    else:
                        state[item["state_key"]] = replay_seed(kind, seed_series, item["params"])
                except Exception as exc:
                    if verbose:
                        warnings.append(f"[X] stateful seed failed for '{kind}': {exc}")
        ...
        if no_rows_after:
            result_df = DataFrame(index=self._df.index).iloc[0:0]
            result_out = self._df if returns else result_df
            result_out = result_out.loc[result_df.index]
            if returns_state:
                return result_out, state
            return result_out
        ...
        if len(work) > 0:
            ...
            for i in row_iter:
                for item in work:
                    ...
                    state[item["state_key"]] = new_state

                    if values is None:
                        continue
                    if not isinstance(values, list):
                        values = list(values)
                    names = item["output_names"]
                    for j, name in enumerate(names):
                        outputs[name][i] = values[j] if j < len(values) else nan

            stateful_df = DataFrame(outputs, index=self._df.index)

        vectorized_results = []
        if len(vectorized_specs) > 0:
            for spec in vectorized_specs:
                kind = spec.get("kind").lower()
                params = spec.get("params", ())
                if not isinstance(params, tuple):
                    params = tuple()
                call_spec = dict(spec)
                call_spec.pop("append", None)
                result = getattr(self, kind)(*params, **call_spec)
                if result is not None:
                    vectorized_results.append(result)

        vectorized_df = DataFrame(index=self._df.index)
        if len(vectorized_results) > 0:
            vectorized_df = concat(vectorized_results, axis=1)

        if len(stateful_df.columns) and len(vectorized_df.columns):
            result_df = concat([stateful_df, vectorized_df], axis=1)
        elif len(stateful_df.columns):
            result_df = stateful_df
        elif len(vectorized_df.columns):
            result_df = vectorized_df
        else:
            result_df = DataFrame(index=self._df.index)

        if state_timestamp is not None and mask is not None:
            result_df = result_df.loc[mask]

        if append and not result_df.empty:
            if state_timestamp is not None and mask is not None:
                for col in result_df.columns:
                    self._df.loc[result_df.index, col] = result_df[col]
            else:
                self._append(result=result_df)
