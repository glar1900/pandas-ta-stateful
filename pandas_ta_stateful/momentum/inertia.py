...
from pandas_ta_stateful._typing import DictLike, Int, IntFloat
from pandas_ta_stateful.overlap import linreg
...
from pandas_ta_stateful.volatility import rvi
...
def inertia(
    ...
    # Safe isnan check - handle object dtypes
    try:
        if all(isnan(rvi_)):
            return  # Emergency Break
    except (TypeError, ValueError):
        # rvi_ has incompatible dtype, try converting to numeric
        try:
            from pandas import to_numeric, Series as PdSeries
            converted = to_numeric(rvi_, errors='coerce')
            if isinstance(converted, PdSeries):
                rvi_ = converted
            if all(isnan(rvi_)):
                return
        except:
    ...
    # Make sure inertia is a Series
    if not isinstance(inertia, Series):
        inertia = Series([0.0] * len(close), index=close.index)
    
    # Safe isnan check for inertia
    try:
        if all(isnan(inertia)):
            return  # Emergency Break
    except (TypeError, ValueError):
        # inertia has incompatible dtype
        try:
            from pandas import to_numeric, Series as PdSeries
            converted = to_numeric(inertia, errors='coerce')
            if isinstance(converted, PdSeries):
                inertia = converted
            if all(isnan(inertia)):
                return
