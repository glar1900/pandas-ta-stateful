...
def _cmo(x: Series, length: Int, drift: Int):
    ...
    Info: Weird Circular TypeError!?
        For some reason: from pandas_ta_stateful.momentum import cmo causes
        pandas_ta_stateful.momentum.coppock to not be able to import it's _wma_ like
        from pandas_ta_stateful.overlap import wma?
    """
    mom = x.diff(drift)
